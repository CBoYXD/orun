...
def test_parallel_consensus_disables_tools_when_requested(monkeypatch) -> None:
    """
    When tools are disabled, no tool definitions should be sent to Ollama.
    """
    ...
    pipeline = {
        "models": [{"name": "function-gemma:2b"}, {"name": "llama3"}],
        "aggregation": {"method": "best_of"},
    }
    ...
    assert all(tool_list is None for tool_list in captured_tools.values())
